...
import { resolveBackAction, type BackNavigationState } from '../src/ui/backNavigation';
...
describe('resolveBackAction', () => {
  ...
  it('ignoriert ein unsichtbares Kursdetail außerhalb des Kalenders', () => {
    expect(resolveBackAction(state({ selectedBlockKey: 'block', section: 'mail' }))).toBe('to-calendar');
  });
  ...
  it('baut den Zustand Ebene für Ebene ab, statt mehrere zu überspringen', () => {
    ...
    expect(resolveBackAction({ ...deep, showSettings: false })).toBe('to-dualis-overview');
    expect(
      resolveBackAction({ ...deep, showSettings: false, selectedBlockKey: null, dualisPage: 'overview' }),
    ).toBe('to-calendar');
  });
});
